...
exports.createReservation = async (req, res) => {
  try {
    ...
    if (isNaN(total_amount) || total_amount <= 0) {
      return res.status(400).json({
        error: lang === 'en' ? 'Invalid total amount' : 'المبلغ الإجمالي غير صالح'
      });
    }
    ...
    if (!['en', 'ar'].includes(lang)) {
      return res.status(400).json({
        error: lang === 'en' ? 'Invalid language. Please use "en" or "ar".' : 'اللغة غير صالحة. استخدم "en" أو "ar".'
      });
    }
  ...
  exports.getReservationById = async (req, res) => {
    try {
      ...
      if (!['en', 'ar'].includes(lang)) {
        return res.status(400).json({
          error: lang === 'en' ? 'Invalid language. Please use "en" or "ar".' : 'اللغة غير صالحة. يرجى استخدام "en" أو "ar".'
        });
      }
      ...
      console.error(error);
      res.status(500).json({
        error: lang === 'en' ? 'Failed to retrieve reservation' : 'فشل في استرجاع الحجز',
      });
    }
  };
  ...
  exports.updateReservation = async (req, res) => {
    try {
      ...
      console.error(error);
      res.status(500).json({
        error: lang === 'en' ? 'Failed to update reservation' : 'فشل في تحديث الحجز',
      });
    }
  };
